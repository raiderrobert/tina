...
reimplements a result-writing path for every system a track targets, and every
...
`result` in a track config (`github:pr`, `github:issue-comment`) is a
...
- Adding a new result system requires no Tina code — only credentials in the image
  and a track that knows how to use them.
